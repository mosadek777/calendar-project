...
- [ ] T-78 [DESIGN] One visual pass across **every** screen — sign-in, sign-up, calendar with
  ...
  **Permitted**: JSX markup, `className` values, the CSS custom properties in `index.css`, a
  Google Font `<link>` in `index.html`, `lucide-react` icons, and `framer-motion` for page
  transitions and list-item enter/exit **only** — the three allowances of Article VII as
  amended in constitution 1.1.0.
  **Forbidden**: handlers, state, fetches, routing, validation, or any other behaviour; any
